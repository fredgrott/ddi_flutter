...
/// ```
class PropertyChangeProvider<T extends PropertyChangeNotifier<S>,
  ...
  static PropertyChangeModel<T, S>?
    ...
    PropertyChangeModel<T, S>? nullCheck(PropertyChangeModel<T, S>? model) {
      assert(model != null,
          'Could not find an ancestor PropertyChangeProvider<$T, $S>');
    ...
    if (!listen) {
      return nullCheck(
          context.findAncestorWidgetOfExactType<PropertyChangeModel<T, S>>(),);
    }

    if (properties == null || properties.isEmpty) {
      return nullCheck(context
          .dependOnInheritedWidgetOfExactType<PropertyChangeModel<T, S>>());
    }

    PropertyChangeModel<T, S>? widget;
    for (final property in properties) {
      widget = InheritedModel.inheritFrom<PropertyChangeModel<T, S>>(context,
          aspect: property,);
    }
  ...
  _PropertyChangeProviderState createState() =>
      _PropertyChangeProviderState<T, S>();
}
...
class _PropertyChangeProviderState<T extends PropertyChangeNotifier<S>,
    S extends Object> extends State<PropertyChangeProvider<T, S>> {
  Set<S> _properties = {};
...
class PropertyChangeModel<T extends PropertyChangeNotifier<S>, S extends Object>
  ...
  bool updateShouldNotifyDependent(
      PropertyChangeModel<T, S> oldWidget, Set<S> aspects,) {
